...
def display_todays_total():
    EM = ExpenseManager()
    EM.from_csv(expense_csv)
    return EM.todays_spending()
    
def display_weekly_total():
    EM = ExpenseManager()
    EM.from_csv(expense_csv)
    return EM.weekly_spending()
    
def display_curr_month_total():
    EM = ExpenseManager()
    EM.from_csv(expense_csv)
    return EM.monthly_spending()
	
    
def display_expense_by_month():
    """ 
    Summarize all expenses by month 
...
def index():  
    return render_template(
        ...
        todaysTotal=display_todays_total(),
        todaysDate=DT.date.today().strftime('%B %d, %Y'),
        todaysDateFmtd=DT.date.today().strftime('%d'),
        weekAgo=(DT.date.today() - DT.timedelta(days=7)).strftime('%B %d'),
        weeklyTotal=display_weekly_total(),
        currMonth=DT.date.today().strftime('%B %Y'),
        monthsTotal=display_curr_month_total()
        )
